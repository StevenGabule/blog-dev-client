...
const UserInfo = () => {
  ...
  const [user, setUser] = useState<IUserProfile>(INITIAL_VALUE)
  ...
  return (
    <form className="profile_info" onSubmit={handleSubmit}>
      ...
      <div className="form-group my-3">
        ...
        <input type="text" className="form-control" id="name" name="name" defaultValue={auth.user.name} onChange={handleChangeInput} />
      </div>

      <div className="form-group my-3">
        <label htmlFor="account">Account</label>
        <input type="text" className="form-control" id="account"
               name="account" defaultValue={auth.user.account} onChange={handleChangeInput} disabled={true} />
      </div>
      {

...
const CardHoriz: React.FC<IProps> = ({blog}) => {
  ...
  const handleDelete = () => {
    ...
    if (slug !== auth.user._id) return dispatch({type: 'ALERT', payload: {errors: 'Invalid Authentication.'}})
    if (window.confirm("Do you want to delete this post?")) dispatch(deleteBlog(blog, auth.access_token))
  }
      ...
      <div className="row g-0 p-2">
        <div className="col-md-4" style={{minHeight: '150px', maxHeight: '170px', overflow: 'hidden'}}>
          {blog.thumbnail && <> {
            typeof (blog.thumbnail) === 'string' ?
              ...
              <img src={URL.createObjectURL(blog.thumbnail)} className="w-100 h-100"
        ...
        <div className="col-md-8">
          <div className="card-body">
            ...
            {blog.title && <p className="card-text d-flex justify-content-between align-items-center">
              {(slug === auth.user?._id) &&
